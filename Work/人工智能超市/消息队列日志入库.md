...
curl -X POST http://127.0.0.1:30016/v1/chat/completions \
  -H "Mass-Auth-Token: ssssssssssssssssss" \
...
ALTER TABLE mq_consume_log
  ADD COLUMN mass_auth_token VARCHAR(255) COMMENT '从 APISIX 日志中提取的认证 Token';
```

```sql
ALTER TABLE mq_consume_log
  MODIFY COLUMN prompt_tokens INT      NOT NULL DEFAULT 0,
  MODIFY COLUMN completion_tokens INT NOT NULL DEFAULT 0,
...
```bash
docker build -t mq2data:0717 .
```

```bash
docker run -p 9090:8080 mq2data:0717
```
...
```bash
java -jar target/mq2data-0.0.1-SNAPSHOT.jar
```

### 创建原米数据库用户

```mysql
CREATE USER 'yuanmi'@'%' IDENTIFIED BY 'yuanmi';
```

下面这条命令赋予了 `yuanmi` 对 `mqdb` 数据库中的 `mq_consume_log` 表的 SELECT 权限，也就是只读访问权限。

```mysql
GRANT SELECT ON mqdb.mq_consume_log TO 'yuanmi'@'%';
```

为了确保权限更改立即生效，执行以下命令刷新权限：
...
```bash
mysql -u yuanmi -p dify
```

```
bash-4.4# mysql -u yuanmi -p dify
Enter password: 
ERROR 1044 (42000): Access denied for user 'yuanmi'@'%' to database 'dify'
```

```bash
mysql -u yuanmi -p mqdb
```

#### 增加权限

```mysql
GRANT ALL PRIVILEGES ON mqdb.mq_consume_log TO 'yuanmi'@'%';

SHOW GRANTS FOR 'yuanmi'@'%';
```
