---
tags: apisix
---

### model_router 插件

```lua
--
--  APISIX Plugin: model_router
--  功能说明：
--    • 普通推理请求：解析 body 中的 `model` 字段 → 动态路由 + 注入 Token。
--    • GET /models（含 /v1/models）：聚合所有上游的模型列表后直接返回。
--      ⚙️ 规则：如果对应 entry.token 中包含子串 "gpustack"，
--               则对子请求路径使用  `/v1-openai/models`，
--               否则使用 `/v1/models`。
--

local core = require("apisix.core")
local http = require("resty.http")      -- HTTP 子请求客户端  :contentReference[oaicite:0]{index=0}

local name = "model_router"

----------------------------------------------------------------------
-- 插件配置 Schema
----------------------------------------------------------------------
local schema = {
    type = "object",
    properties = {
        body_key = { type = "string", default = "model" },
        mapping  = {
            type              = "object",
            minProperties     = 1,
            additionalProperties = {
                type       = "object",
                required   = { "target" },   -- host:port
                properties = {
                    target  = { type = "string" },
                    token   = { type = "string" },
                    timeout = { type = "integer", minimum = 100, default = 2000 }
                }
            }
        }
    },
    required = { "mapping" }
}

----------------------------------------------------------------------
local _M = {
    version  = 0.5,
    priority = 500,
    name     = name,
    schema   = schema,
}

----------------------------------------------------------------------
-- 辅助：按 token 内容决定拉取路径并返回 list 数据
----------------------------------------------------------------------
local function fetch_models(entry)
    local host, port = core.utils.parse_addr(entry.target)       -- 解析 host:port :contentReference[oaicite:1]{index=1}
    if not host or not port then
        return nil, "invalid target: " .. tostring(entry.target)
    end

    local path = (entry.token and entry.token:find("gpustack", 1, true))
                  and "/v1-openai/models"
                  or "/v1/models"

    local httpc = http.new()
    httpc:set_timeout(entry.timeout or 2000)

    local res, err = httpc:request_uri(
        "http://" .. host .. ":" .. port .. path,
        {
            method  = "GET",
            headers = entry.token and { Authorization = entry.token } or nil
        }
    )

    if not res or res.status >= 300 then
        return nil, err or ("status: " .. (res and res.status or "nil"))
    end

    local body = core.json.decode(res.body)
    if body and body.data then
        return body.data
    end

    return nil, "no data[] in body"
end

----------------------------------------------------------------------
-- rewrite 阶段
----------------------------------------------------------------------
function _M.rewrite(conf, ctx)
    ------------------------------------------------------------------
    -- A. 处理 GET /models | /v1/models
    ------------------------------------------------------------------
    local uri, method = ngx.var.uri, ngx.req.get_method()

    core.log.warn(name, " [rewrite] uri: ", uri, " method: ", method)

    if method == "GET" and (uri == "/models" or uri == "/v1/models") then
        core.log.warn(name, " [rewrite] aggregating /models list")

        local visited, merged = {}, {}

        for _, entry in pairs(conf.mapping) do
            local sig = entry.target .. "|" .. (entry.token or "")
            if not visited[sig] then
                visited[sig] = true

                local list, err = fetch_models(entry)
                if not list then
                    core.log.error(name, " [models] fetch failed from ", entry.target, ": ", err)    
                else
                    for _, m in ipairs(list) do
                        merged[m.id or (#merged + 1)] = m
                    end
                end
            end
        end

        local data, i = {}, 0
        for _, m in pairs(merged) do
            i = i + 1
            data[i] = m
        end

        core.log.warn(name, " [models] final size=", #data)
        -- 直接返回聚合结果（rewrite 阶段可 return status + body） :contentReference[oaicite:2]{index=2}
        return 200, { object = "list", data = data }
    end

    ------------------------------------------------------------------
    -- B. 解析普通推理请求中的 model 字段
    ------------------------------------------------------------------
    local body, err = core.request.get_body(nil, ctx)
    if not body then
        core.log.error(name, " [rewrite] failed to get body: ", err)
        return
    end

    local decoded, jerr = core.json.decode(body)
    if not decoded then
        core.log.error(name, " [rewrite] json decode error: ", jerr)
        return
    end

    local model = tostring(decoded[conf.body_key] or "")
    local entry = conf.mapping[model]
    if not entry then
        core.log.warn(name, " [rewrite] no entry for model: ", model)
        return
    end

    ctx.model_router_entry = entry
end

----------------------------------------------------------------------
-- before_proxy 阶段：动态设置上游并注入 Authorization
----------------------------------------------------------------------
function _M.before_proxy(conf, ctx)
    local entry = ctx.model_router_entry

    core.log.warn("[before_proxy] entry = ", core.json.encode(entry))

    if not entry then
        return   -- 默认上游
    end

    local host, port = core.utils.parse_addr(entry.target)
    if not host or not port then
        core.log.error(name, " [before_proxy] invalid target: ", entry.target)
        return
    end

    core.log.warn(name, " [before_proxy] setting upstream to ", host, ":", port)

    ----------------------------------------------------------------------
    -- ✅ 1. 真正设置 ctx.upstream，让 APISIX 转发到新目标
    ----------------------------------------------------------------------
    ctx.upstream = {
        type = "roundrobin",
        scheme = "http",
        pass_host = "pass",
        nodes = {
            [host .. ":" .. port] = 1
        }
    }

    ----------------------------------------------------------------------
    -- ✅ 2. 更新 matched_route.value.upstream.nodes 中的字段
    -- 注意：这是修改原始路由配置的副本，对后续插件和日志可见
    ----------------------------------------------------------------------
    local matched_upstream = ctx.matched_route
        and ctx.matched_route.value
        and ctx.matched_route.value.upstream

    if matched_upstream and matched_upstream.nodes then
        for _, node in ipairs(matched_upstream.nodes) do
            node.host = host
            node.port = port
            core.log.warn(name, " [before_proxy] updated matched_route upstream node to ", host, ":", port)
        end
    end

    ----------------------------------------------------------------------
    -- ✅ 3. 注入 Authorization Header（如配置了 token）
    ----------------------------------------------------------------------
    if entry.token then
        core.request.set_header(ctx, "Authorization", entry.token)
        core.log.warn(name, " [before_proxy] injected Authorization header")
    end

    ----------------------------------------------------------------------
    -- 🔍 最后打印 ctx，确保设置生效
    ----------------------------------------------------------------------
    core.log.warn(" [before_proxy] final ctx = ", core.json.encode(ctx, true))
end

return _M
```



### 流程图

```
sequenceDiagram

    autonumber

    participant Client as 客户端

    participant Gateway as 统一模型网关

    participant ForwardAuth as forward-auth 插件

    participant AuthService as 原米鉴权服务

    participant ModelRouter as model-router 插件

    participant Upstream as 上游模型服务 (Qwen3 / DeepSeek / etc.)

    participant RocketMQ as rocketmq 插件 (消息上报)

  

    Client->>Gateway: POST /v1/chat/completions<br/>Header: Authorization: 原米鉴权服务需要的 token

    Note right of Client: 客户端携带 Authorization 头发起请求

  

    %% 鉴权阶段

    Gateway->>ForwardAuth: 调用 forward-auth 插件

    ForwardAuth->>AuthService: 请求鉴权接口 (携带 Authorization Token)

    AuthService-->>ForwardAuth: 返回鉴权结果 (允许 / 拒绝)

    ForwardAuth-->>Gateway: 鉴权通过 / 拒绝响应

    Note right of ForwardAuth: 若鉴权失败，直接终止请求

  

    %% 模型路由阶段

    Gateway->>ModelRouter: 调用 model-router 插件

    ModelRouter->>ModelRouter: 从请求体中解析 "model" 字段

    ModelRouter->>ModelRouter: 查询映射表 (model → upstream + token)

    ModelRouter->>Gateway: 返回上游信息 & 替换 Header<br/>Authorization → 上游真实 token

    Note right of ModelRouter: 插件根据模型名<br/>决定上游服务和授权方式

  

    %% 转发到上游

    Gateway->>Upstream: 转发请求<br/>Header: Authorization: <上游真实token>

    Upstream-->>Gateway: 返回模型响应 (结果内容)

  

    %% RocketMQ 异步上报阶段

    par 主流程继续

        Gateway-->>Client: 返回模型最终响应

        Note right of Gateway: 无论上报成功与否<br/>都立即返回响应给客户端

    and 异步上报

        Gateway->>RocketMQ: 调用 rocketmq 插件上报请求日志 / 指标

        RocketMQ-->>Gateway: 上报结果 (成功 / 失败)

        Note right of RocketMQ: 上报失败不影响主流程

    end
```

### 管理路由配置

```bash
curl -X PUT http://127.0.0.1:30015/apisix/admin/routes/llm \
  -H "X-API-KEY: edd1c9f034335f136f87ad84b625c8f1" -d '
{
  "uri": "/v1/*",
  "name": "统一大模型入口",
  "plugins": {
    "forward-auth": {
      "allow_degradation": false,
      "client_headers": [
        "Location"
      ],
      "keepalive": true,
      "keepalive_pool": 5,
      "keepalive_timeout": 60000,
      "request_headers": [
        "Authorization",
        "Mass-Auth-Token",
        "Mass-Ai-Service"
      ],
      "request_method": "GET",
      "ssl_verify": true,
      "status_on_error": 403,
      "timeout": 3000,
      "upstream_headers": [
        "X-User-ID",
        "Authorization"
      ],
      "uri": "http://192.168.120.40:7070/tool/apisix/massAuthentication"
    },
    "model_router": {
      "body_key": "model",
      "mapping": {
        "QwQ-32B": {
          "target": "192.168.80.100:38080",
          "token": "Bearer sk-lKn0TBxjwG8InOpoTvipKAghtuMKT43XSRo9wNZJjmME7M0D"
        },
        "DeepSeek-R1-Llama-70B": {
          "target": "192.168.80.100:38080",
          "token": "Bearer sk-lKn0TBxjwG8InOpoTvipKAghtuMKT43XSRo9wNZJjmME7M0D"
        },
        "Qwen2.5-72B-Instruct": {
          "target": "192.168.80.100:38080",
          "token": "Bearer sk-lKn0TBxjwG8InOpoTvipKAghtuMKT43XSRo9wNZJjmME7M0D"
        },

        "DeepSeek-R1-0528-Qwen3-8B": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Lingshu-32B": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen2-Audio-7B": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen2.5-32B-Instruct": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen3-30B-A3B-GPTQ-Int4": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen3-235B-A22B-GPTQ-Int4": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen3-Embedding-8B": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "bge-m3": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "bge-reranker": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        }
      }
    }
  },
  "plugin_config_id": "rocketmq-log",
  "upstream": {
    "nodes": [
      {
        "host": "192.168.120.44",
        "port": 9998,
        "weight": 1
      }
    ],
    "type": "roundrobin",
    "hash_on": "vars",
    "scheme": "http",
    "pass_host": "pass"
  },
  "status": 1
}'
```

超算中心模型列表查询

```bash
curl -X GET "http://192.168.80.100:38080/v1/models" \
  -H "Authorization: Bearer sk-lKn0TBxjwG8InOpoTvipKAghtuMKT43XSRo9wNZJjmME7M0D"
```

加了平台的鉴权

```bash
curl -X POST http://127.0.0.1:40000/v1/chat/completions \
  -H "Mass-Auth-Token: afeadf3adc4f7ab2949905f91eee47a6" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "Qwen3-235B-A22B-GPTQ-Int4",
    "messages": [
      {"role": "user", "content": "你好"}
    ]
  }'
```

鉴权 token 缺失

```bash
curl -X POST http://127.0.0.1:40000/v1/chat/completions \
  -H "Mass-Auth-Token: a6" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "DeepSeek-R1-0528-Qwen3-8B",
    "messages": [
      {"role": "user", "content": "你好"}
    ]
  }'
```

![[Pasted image 20250912152904.png]]

0912 时候测试一切正常

### 0917

```shell
curl -X POST http://10.17.101.74:9090/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer token-abc123" \
  -d '{
    "model": "Qwen3-Next-80B-A3B-Instruct",
    "messages": [
      {"role": "user", "content": "你好"}
    ],
    "stream": true 
  }'
```

![[Pasted image 20250917150549.png]]

```shell
curl -X POST http://127.0.0.1:40000/v1/chat/completions \
  -H "Mass-Auth-Token: afeadf3adc4f7ab2949905f91eee47a6" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "Qwen3-Next-80B-A3B-Instruct",
    "messages": [
      {"role": "user", "content": "你好"}
    ]
  }'
```

![[Pasted image 20250917150928.png]]

![[Pasted image 20250917150959.png]]

#### 添加脚本

```shell
curl -X PUT http://127.0.0.1:30015/apisix/admin/routes/llm \
  -H "X-API-KEY: edd1c9f034335f136f87ad84b625c8f1" -d '
{
  "uri": "/v1/*",
  "name": "统一大模型入口",
  "plugins": {
    "forward-auth": {
      "allow_degradation": false,
      "client_headers": [
        "Location"
      ],
      "keepalive": true,
      "keepalive_pool": 5,
      "keepalive_timeout": 60000,
      "request_headers": [
        "Authorization",
        "Mass-Auth-Token",
        "Mass-Ai-Service"
      ],
      "request_method": "GET",
      "ssl_verify": true,
      "status_on_error": 403,
      "timeout": 3000,
      "upstream_headers": [
        "X-User-ID",
        "Authorization"
      ],
      "uri": "http://192.168.120.40:7070/tool/apisix/massAuthentication"
    },
    "model_router": {
      "body_key": "model",
      "mapping": {
	    "Qwen3-Next-80B-A3B-Instruct": {
		  "target": "10.17.101.74:9090",
		  "token": "Bearer token-abc123"
	    },
        "QwQ-32B": {
          "target": "192.168.80.100:38080",
          "token": "Bearer sk-lKn0TBxjwG8InOpoTvipKAghtuMKT43XSRo9wNZJjmME7M0D"
        },
        "DeepSeek-R1-Llama-70B": {
          "target": "192.168.80.100:38080",
          "token": "Bearer sk-lKn0TBxjwG8InOpoTvipKAghtuMKT43XSRo9wNZJjmME7M0D"
        },
        "Qwen2.5-72B-Instruct": {
          "target": "192.168.80.100:38080",
          "token": "Bearer sk-lKn0TBxjwG8InOpoTvipKAghtuMKT43XSRo9wNZJjmME7M0D"
        },

        "DeepSeek-R1-0528-Qwen3-8B": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Lingshu-32B": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen2-Audio-7B": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen2.5-32B-Instruct": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen3-30B-A3B-GPTQ-Int4": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen3-235B-A22B-GPTQ-Int4": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "Qwen3-Embedding-8B": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "bge-m3": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        },
        "bge-reranker": {
          "target": "192.168.120.44:9998",
          "token": "Bearer gpustack_4f50a5c7bad4fd01_7c054bcb8d3cf921e9b1fa43517fa81b"
        }
      }
    }
  },
  "plugin_config_id": "rocketmq-log",
  "upstream": {
    "nodes": [
      {
        "host": "192.168.120.44",
        "port": 9998,
        "weight": 1
      }
    ],
    "type": "roundrobin",
    "hash_on": "vars",
    "scheme": "http",
    "pass_host": "pass"
  },
  "status": 1
}'
```

### 1013

```shell
curl -X POST http://127.0.0.1:40000/v1/chat/completions \
  -H "Mass-Auth-Token: 2311480e5b1bda8a4a572dc0ff6607df" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "Qwen3-Next-80B-A3B-Instruct",
    "messages": [
      {"role": "user", "content": "你好"}
    ]
  }'
```

```shell
curl -X POST http://127.0.0.1:40000/v1/chat/completions \
  -H "Authorization: 2311480e5b1bda8a4a572dc0ff6607df" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "Qwen3-235B-A22B-GPTQ-Int4",
    "messages": [
      {"role": "user", "content": "你好"}
    ]
  }'
```

```shell
curl -X POST http://127.0.0.1:40000/v1/chat/completions \
  -H "Authorization: 26607df" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "Qwen3-Next-80B-A3B-Instruct",
    "messages": [
      {"role": "user", "content": "你好"}
    ]
  }'
```

```shell
curl -X POST http://127.0.0.1:30016/v1/chat/completions \
  -H "Authorization: afeadf3adc4f7ab2949905f91eee47a6" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "Qwen3-Next-80B-A3B-Instruct",
    "messages": [
      {"role": "user", "content": "你好"}
    ]
  }'
```

```
curl -X GET "127.0.0.1:40000/v1/models" \
  -H "Authorization: 2311480e5b1bda8a4a572dc0ff6607df"
```

---
### 1014
#### dify 接入测试

![[a37defaf-433a-4a7f-b94c-4dd69601304b.png]]


![[image.png]]


```shell
curl -X POST http://127.0.0.1:40000/v1/chat/completions \
  -H "Authorization: Bearer 2311480e5b1bda8a4a572dc0ff6607df" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "Qwen3-235B-A22B-GPTQ-Int4",
    "messages": [
      {"role": "user", "content": "你好"}
    ]
  }'
```

```shell
curl -X POST http://127.0.0.1:40000/v1/chat/completions \
  -H "Authorization: 2311480e5b1bda8a4a572dc0ff6607df" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "Qwen3-235B-A22B-GPTQ-Int4",
    "messages": [
      {"role": "user", "content": "你好"}
    ]
  }'
```