...
OAuth2.0 是一套由 IETF 发布，行业广泛采用的授权框架（Authorization Framework），其核心目标是让 **第三方应用** 在不直接获得用户的账号密码的前提下，安全地访问用户在其他服务（资源服务器）上受到保护的资源。

#### 自家认证和 Oauth2 的区别

前端直接把用户名/密码发自家后端，后端校验数据库，签一个 JWT（含用户 id/角色），这种典型的认证场景属于 “自家认证”。

在自家认证的逻辑中常常会将 scope 忽略，因为这种场景下我们只需要知道这个人是谁 + 是否登录。

典型的流程是：后端验证后签发一个 JWT，前端后续请求中带上这个 JWT，授权逻辑直接看 role 或者是内部逻辑表。

这里的业务授权粒度（角色/权限表）已经在我们的后端中了；**OAuth 的 scope 抽象并没有出现**。  

换句话说：我们使用的是 **“内部权限系统”**，而不是 **“对外授权契约”**。

**scope 真正的价值** 出现在：

- 需要第三方客户端调用你的 API（GitHub Apps、微信小程序调用平台 API）。
    
- 需要让用户“同意”哪些资源能被外部应用访问。
    
- 需要多个微服务/资源服务器共享一个统一的授权服务器产出的令牌。
    
- 需要 _最小权限_、差异化 token（读写分离、支付操作分离）。
...
- **Resource Owner Password Credentials（密码模式）**  
    用户将用户名/密码直接给客户端，由客户端向授权服务器换取令牌。只建议在高信任环境（如自己开发的官方 App）中使用。可以带 `scope` 参数，由 AS 决定授予哪些 scope
